...
public class NN_education {
	public static void main(String[] args) {
		...
		double[][] gdWeights1 = new double[3][6];
		double[] gdWeights2 = new double[4];
		double[][] stWeights1 = new double[3][6];
		...
		try {
			...
			while(trainLsc.hasNextLine()) {
					real_outputs.add(Double.parseDouble(trainLsc.nextLine())/ 100);
			}
			trainLsc.close();
			...
			while(trainsc.hasNextLine()) {
				if(columnNames) {
					...
					for(int i = 1; i < inputs.length; i++) {
						inputs[i] = (Double.parseDouble(inputValues[i-1])) / 100;
					}
		...
		try {
			...
			while(testsc.hasNextLine()) {
				...
				//	real_test_out = Double.parseDouble(testLsc.nextLine()) - 100;
				//}
				if(firstline) {
					firstline = false;
					testsc.nextLine();
					...
					for(int i = 1; i < testinputs.length; i++) {
						testinputs[i] = Double.parseDouble(testinputValues[i-1])/ 100;
					}
					...
					System.out.println(test_out*100);
				}
			}
